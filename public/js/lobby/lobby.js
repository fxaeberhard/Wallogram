        ...
        initUI: function() {
            ...
            $.getJSON("/levels/getAllLevels", null, function(levels) {
                $.each(levels, function(key, value) {
                    if (key % 4 == 0 || key == 0) {
                        $('.wallo-load-levels').append('<div class="row"><article class="col-md-3 level" id="' + value._id + '">'
                            + '<img src="assets/default-game-sample.png" alt="level thumbnail"/><div class="level-infos"><h5>' + value.name + '</h5>'
                            + '<span>3 hours ago</span><div class="buttons">'
                            + '<a href="/screen?level=' + value.name + '" class="levelTreatment edit fa fa-play"></a>'
                            + '<a href="/screen?level=' + value.name + '&edit=true" class="levelTreatment edit fa fa-pencil-square-o"></a>'
                            + '<a href="#" class="levelTreatment delete fa fa-trash"></a></div></div></article></div>')
                    } else {
                        $('.wallo-load-levels .row:last-child').append('<article class="col-md-3 level" id="' + value._id + '">'
                            + '<img src="assets/default-game-sample.png" alt="level thumbnail"/><div class="level-infos"><h5>' + value.name + '</h5>'
                            + '<span>3 hours ago</span><div class="buttons">'
                            + '<a href="/screen?level=' + value.name + '" class="levelTreatment edit fa fa-play"></a>'
                            + '<a href="/screen?level=' + value.name + '&edit=true" class="levelTreatment edit fa fa-pencil-square-o"></a>'
                            + '<a href="#" class="levelTreatment delete fa fa-trash"></a></div></div></article>')
                    }
